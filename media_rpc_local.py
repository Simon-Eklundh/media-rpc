...
def check_environment_variables():
    if not JELLYFIN_SERVER:
        print(f"missing ${JELLYFIN_SERVER} in .env")
        raise EnvironmentError(
            f"Missing required environment variable: {JELLYFIN_SERVER}"
        )
    ...
    if not JELLYFIN_API_KEY:
        print(f"missing ${JELLYFIN_API_KEY} in .env")
        raise EnvironmentError(
            f"Missing required environment variable: {JELLYFIN_API_KEY}"
        )
    if not JELLYFIN_USER_ID:
        print(f"missing ${JELLYFIN_USER_ID} in .env")
        raise EnvironmentError(
            f"Missing required environment variable: {JELLYFIN_USER_ID}"
        )
    if not TMDB_API_KEY:
        print(f"missing ${TMDB_API_KEY} in .env")
        raise EnvironmentError(f"Missing required environment variable: {TMDB_API_KEY}")
    if not ABS_SERVER:
        print(f"missing ${ABS_SERVER} in .env")
        raise EnvironmentError(f"Missing required environment variable: {ABS_SERVER}")
    if not ABS_API_TOKEN:
        print(f"missing ${ABS_API_TOKEN} in .env")
        raise EnvironmentError(
            f"Missing required environment variable: {ABS_API_TOKEN}"
        )
