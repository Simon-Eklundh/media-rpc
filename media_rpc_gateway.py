...
from discord_api import client_properties
from discord_api.gateway import Gateway
...
def fetch_jellyfin():
    try:
        res = requests.get(
            JELLYFIN_SERVER, headers={"X-Emby-Token": JELLYFIN_API_KEY}, timeout=1
        ).json()
        if not res:
            return None
        session = next(
            (
                ...
                and s.get("UserId") == JELLYFIN_USER_ID
            ),
            None,
        )
        if not session:
        ...
        title = item.get("Name")
        artist_name = "Oakgrove"  # can be changed
        item_id = item.get("Id")
        ...
        match client:
            ...
            case "Kodi":
                small_icon = "https://images.icon-icons.com/1495/PNG/512/kodi_102964.png"
            case "Wholphin":
...
def fetch_abs():
    try:
        ...
        res = requests.get(
            url, headers={"Authorization": f"Bearer {ABS_API_TOKEN}"}, timeout=2
        ).json()
...
def sigint_handler(_signum, _frame):
    global gateway
    if gateway and gateway.get_state() == 1:
        gateway.update_presence("online", activities=[])
...
def main():
    ...
    # this is a fake last_printed value to make sure the idle state is set on first run. if nothing is running
    last_printed = "Fake_Initial_Value"
    ...
    while True:
        ...
        if data:
            ...
            activity = {
                ...
                "state": data["state"],
                "assets": {
                    "large_image": resolve_mp_url(data["cover"]),
                    "large_text": data["text"],
                    "small_image": resolve_mp_url(small_icon),
                    "small_text": "Playing",
                },
                "type": data["type"],
            ...
            if gateway.get_state() == 1:
                gateway.update_presence(data["status"], activities=[activity], afk=False)
        else:
            if last_printed is not None:
                print("\n[RPC] Idle")
                last_printed = None
                if gateway.get_state() == 1:
                    gateway.update_presence("idle", activities=[], afk=True)

        if gateway.error:
            print(f"Gateway error: {gateway.error}, reconnecting...")
            gateway = setup_gateway()
